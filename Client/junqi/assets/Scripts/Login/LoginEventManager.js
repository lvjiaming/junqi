...
const LoginEventManager = cc.Class({
    extends: GameEventManager,
    statics: {
        getInstance() {
            if (!this.loginEventManager) {
    ...
    connectServer(hostStr, callBack) {
        cc.log(`loginEventManager connect`);
        this.connect(hostStr, callBack);
    },
    startEvent(event, data) {
        cc.log(`发送的协议id为：${event}`);
        switch (event) {
            case LoginEventManager.Event.EVENT_LOGIN_REQ: {
                this.sendMessage(event, data);
                break;
            }
            case LoginEventManager.Event.EVENT_ENTER_ROOM_REQ: {
                this.sendMessage(event, data);
                break;
            }
            case LoginEventManager.Event.EVENT_LEAVE_ROOM_REQ: {
                this.sendMessage(event, data);
                break;
            }
            case LoginEventManager.Event.EVENT_SEND_MEG_REQ: {
                this.sendMessage(event, data);
                break;
            }
            case LoginEventManager.Event.EVENT_CREATE_ROOM_REQ: {
                this.sendMessage(event, data);
                break;
            }
            default: {
    ...
    onMsg(msgId, msgData) {
        ...
        switch (msgId) {
            case LoginEventManager.Event.EVENT_LOGIN_REP: {
                // const msg = cc.dd.basePb.Login_rep.deserializeBinary(msgData);  //  反序列化数据
                cc.dd.UserData.setMsgData(msgData);
                break;
            }
            case LoginEventManager.Event.EVENT_ENTER_ROOM_REP: {
                cc.dd.RoomData.enterRoom(msgData);
                break;
            }
            case LoginEventManager.Event.EVENT_LEAVE_ROOM_REP: {
                this.notifyEvent(msgId, msgData);
                break;
            }
            case LoginEventManager.Event.EVENT_SEND_MEG_REP: {
                this.notifyEvent(msgId, msgData);
                break;
            }
            case LoginEventManager.Event.EVENT_CREATE_ROOM_REP: {
                cc.dd.RoomData.setMsg(msgData);
                break;
            }
            case LoginEventManager.Event.EVENT_MESSAGE_PUSH: {
                this.notifyEvent(msgId, msgData);
                break;
            }
            case LoginEventManager.Event.USER_ENTER_ROOM_PUSH: {
                cc.dd.RoomData.otherPlayerEnter(msgData);
                break;
            }
            case LoginEventManager.Event.USER_LEAVE_ROOM_PUSH: {  //  玩家离开的推送
...
cc.loginEventM = LoginEventManager.getInstance();
